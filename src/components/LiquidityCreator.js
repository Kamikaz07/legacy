...
const LiquidityCreator = ({ publicKey }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.solAmount || isNaN(solNum) || solNum <= 0) newErrors.solAmount = 'SOL amount must be a positive number';
    const tokenNum = Number(formData.tokenAmount);
    if (!formData.tokenAmount || isNaN(tokenNum) || tokenNum <= 0) newErrors.tokenAmount = 'Token amount must be a positive number';
    setErrors(newErrors);
  ...
  const handleCreatePool = async () => {
    ...
    setResult(null);
    try {
      const response = await axios.post('http://localhost:3001/api/create-liquidity-pool', {
  ...
  return (
    <ChaosPaper elevation={3}>
      <Typography variant="h4" gutterBottom>Add Liquidity</Typography>
      <Box component="form" onSubmit={(e) => e.preventDefault()}>
        ...
        <ChaosButton
          ...
          fullWidth
        >
          {loading ? <CircularProgress size={24} color="inherit" /> : 'Create Liquidity Pool'}
        </ChaosButton>
      </Box>
      {result && (
        <Box mt={3}>
          {result.success ? (
            ...
            <>
              ...
              {result.details && <Typography color="error">Details: {result.details}</Typography>}
            </>
          )}
        </Box>
      )}
