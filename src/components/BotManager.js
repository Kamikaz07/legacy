...
const BotManager = () => {
  ...
  const fetchTokenTransactions = async (tokenMint) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/token-transactions`,
        {
          params: {
            tokenMint,
            network,
  ...
  const fetchBotStatus = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/bot-status`,
        {
          params: { network },
        },
      );
  ...
  const startBots = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/start-bots`,
        {
          ...formData,
      ...
      if (response.data.success) {
        ...
        setMessage({
          text: response.data.message || "Falha ao iniciar bots.",
          type: "error",
        });
      }
    } catch (error) {
      ...
      setMessage({
        text:
          ...
          "Erro ao iniciar bots. Verifique o console para detalhes.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const stopBot = async (tokenMint) => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/stop-bot`,
        {
          tokenMint,
          network,
        },
      ...
      if (response.data.success) {
        ...
        setMessage({
          text: response.data.message || "Falha ao parar bot.",
          type: "error",
        });
      }
    } catch (error) {
      ...
      setMessage({
        text: "Erro ao parar bot. Verifique o console para detalhes.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const stopAllBots = async () => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/stop-all-bots`,
        {
          network,
        },
      );
      ...
      if (response.data.success) {
        setMessage({
          text: "Todos os bots foram parados com sucesso!",
          type: "success",
        });
        fetchBotStatus();
      } else {
        setMessage({
          text: response.data.message || "Falha ao parar todos os bots.",
          type: "error",
        });
      }
    } catch (error) {
      ...
      setMessage({
        text: "Erro ao parar todos os bots. Verifique o console para detalhes.",
        type: "error",
      });
    } finally {
      setLoading(false);
                  ...
                  <TableBody>
                    {activeBots.map((bot) => (
                      <TableRow key={bot.tokenMint}>
                        <TableCell>
                          {getTokenNameByMint(bot.tokenMint)}
                        </TableCell>
                        <TableCell>
                          {(bot.minDelay / 1000).toFixed(0)} -{" "}
                          {(bot.maxDelay / 1000).toFixed(0)} seg
                        </TableCell>
                        <TableCell>
                        ...
                        >
                          ...
                          <TableCell align="right">
                            {formatAmount(
                              tx.amount,
                              tx.decimals,
                              tokens.find((t) => t.mint === formData.tokenMint)
                                ?.symbol,
                            )}
                          </TableCell>
                          <TableCell align="right">
                          ...
                          <TableCell align="center">
                            <Tooltip
                              title={
                                isBot ? "Transação do Bot" : "Transação Externa"
                              }
                            >
                              <IconButton
                                size="small"
                                sx={{ color: isBot ? "#92E643" : "#888" }}
                              >
                                {isBot ? (
        ...
        <Grid item xs={12}>
          ...
          >
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              • Os bots compram e vendem automaticamente o token selecionado em
              intervalos aleatórios.
            </Typography>
            <Typography
              variant="body2"
              sx={{ mb: 1, fontWeight: "bold", color: "#FF6B6B" }}
            >
              • Na Devnet, as transações são simuladas pois não é possível criar
              liquidity pools no Raydium.
            </Typography>
            <Typography
              variant="body2"
              sx={{
            ...
            >
              ...
              {network === "mainnet-beta"
                ? "MAINNET (usa SOL real)"
                : network.toUpperCase()}
            </Typography>
            <Typography variant="body2" sx={{ mb: 1 }}>
              • As carteiras utilizadas são as mesmas que aparecem no
            ...
            <Typography variant="body2" sx={{ mb: 1 }}>
              ...
              <SmartToyIcon
                fontSize="small"
                sx={{ color: "#92E643", verticalAlign: "middle" }}
              />{" "}
              indica transações dos bots e{" "}
              <PersonIcon
                fontSize="small"
                sx={{ color: "#888", verticalAlign: "middle" }}
              />{" "}
              indica transações de outros usuários.
            </Typography>
            <Typography
              variant="body2"
              sx={{ mb: 1, fontStyle: "italic", color: "#FF6B6B" }}
            >
              • Na Devnet, as transações são apenas para teste e não refletem
              trocas reais no Raydium.
            </Typography>
          </Paper>
        </Grid>
      </Grid>
