...
const Vortex = styled('div')({
  ...
  height: '100px',
  borderRadius: '50%',
  background: 'conic-gradient(from 0deg, #92E643, #39ff14, #92E643, #39ff14, #92E643)',
  ...
  '&::after': {
    ...
    bottom: '20%',
    borderRadius: '50%',
    background: 'conic-gradient(from 180deg, #92E643, #39ff14, #92E643, #39ff14, #92E643)',
...
const Mixer = () => {
  ...
  const handleMixFunds = async () => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/mix-funds`, {
        destinationAddress,
        amountInSol,
      });
      ...
      setErrorMessage('Failed to initiate mixing: ' + (error.response?.data?.error || error.message));
      setLoading(false);
    }
  };
  ...
  const pollMixingStatus = (id) => {
    const intervalId = setInterval(async () => {
      try {
        const statusResponse = await axios.get(`${process.env.REACT_APP_API_URL}/api/mix-status/${id}`);
        ...
        if (data.status === 'completed') {
          setSuccessMessage(data.message || 'Funds mixed successfully through multiple hops!');
          setLoading(false);
    ...
    <ChaosPaper>
      ...
      {loading ? (
        <Box sx={{ textAlign: 'center' }}>
          ...
          {mixingSteps.length > 0 && (
            <StyledScrollBox sx={{ textAlign: 'left', mt: 2, p: 2, bgcolor: 'rgba(0,0,0,0.3)', borderRadius: 1 }}>
              ...
              {mixingSteps.map((step, index) => (
                <Typography 
                  ...
                  variant="body2" 
                  sx={{ 
                    color: step.includes('hop') ? '#FFD700' : 
                           step.includes('Decoy') ? '#FF7F50' : '#fff', 
                    fontSize: '0.85rem',
        ...
        <Box component="form" sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <ActionButton
            ...
            disabled={loading || !!addressError || !!amountError || !destinationAddress || !amountInSol}
          >
            Mix Funds
          </ActionButton>
        </Box>
      ...
      {successMessage && (
        <Box sx={{ mt: 2 }}>
          ...
          {signatures.length > 0 && (
            <StyledScrollBox sx={{ mt: 1, p: 1, bgcolor: 'rgba(0,0,0,0.2)', borderRadius: 1 }}>
              <Typography variant="body2" sx={{ color: '#fff', mb: 1 }}>
