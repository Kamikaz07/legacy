...
const WalletManager = () => {
  ...
  // Carregar carteiras ao montar o componente ou quando a rede mudar
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    fetchWallets();
  }, [network]);
  ...
  const generateWallets = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/generate-wallets`,
        {
      ...
      if (response.data.fromDatabase && response.data.newlyCreated === 0) {
        ...
        setSuccessMessage(
          `Criadas ${response.data.newlyCreated} novas carteiras. Total: ${response.data.wallets.length}`,
        );
      } else {
  ...
  const fundWallets = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/fund-wallets`,
        {
      ...
      for (const txBase64 of transactions) {
        try {
          ...
          setSuccessMessage(
            `Processing: ${completedBatches}/${batches} batches complete (${progress}%)`,
          );
        } catch (txError) {
      ...
      if (completedBatches === batches) {
        setSuccessMessage(
          "Transactions completed. Waiting for blockchain propagation (5 seconds)...",
        );
        ...
        setSuccessMessage(
          `${solAmount} SOL distributed successfully! Each wallet received approximately ${amountPerWallet.toFixed(4)} SOL.`,
        );
      } else {
        setError(
          `Apenas ${completedBatches} de ${batches} lotes foram processados. Verificando saldos atualizados...`,
        );
      ...
      try {
        await updateBalancesInDatabase();
        setSuccessMessage(
          "Saldos atualizados após erro. Verifique se as transações foram confirmadas.",
        );
      } catch (updateError) {
  ...
  const buyTokens = async () => {
    ...
    if (!connection) {
      setError(
        "Wallet connection not available. Please reconnect your wallet.",
      );
      return;
    }
    ...
    const selectedTokenInfo = masterTokens.find(
    ...
    if (amountPerWallet <= 0) {
      setError(
        `Saldo insuficiente para distribuir ${percentage}% entre ${wallets.length} carteiras`,
      );
      return;
    }
    ...
    try {
      ...
      setSuccessMessage(
        `Preparando distribuição de ${percentage}% dos tokens (${totalToDistribute.toFixed(2)} ${selectedTokenInfo.symbol}) para ${wallets.length} carteiras...`,
      );

      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/buy-tokens`,
        {
      ...
      const { transactions, totalTokens, batches, tokenSymbol, tokenName } =
        response.data;
      let completedBatches = 0;
      ...
      setSuccessMessage(
        `Preparado para enviar ${totalTokens.toFixed(2)} ${tokenSymbol} para ${wallets.length} carteiras em ${batches} lotes.`,
      );
      ...
      for (const [index, txBase64] of transactions.entries()) {
        try {
          setSuccessMessage(
            `Processando lote ${index + 1} de ${transactions.length}...`,
          );
          ...
          setSuccessMessage(
            `Progress: ${completedBatches}/${batches} batches completed (${progress}%)...`,
          );
        } catch (txError) {
          console.error("Transaction error:", txError);
          setError(
  ...
  const sellAllTokens = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        `Preparing to collect all ${tokenSymbol} and SOL from shadow wallets...`,
      );

      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/sell-tokens`,
        {
          tokenAddress: selectedToken,
          masterAddress: publicKey.toString(),
      ...
      if (walletsWithTokens > 0) {
        collectionSummary.push(
          `${totalTokensCollected} ${responseSymbol} from ${walletsWithTokens} wallets`,
        );
      }
      if (walletsWithSol > 0) {
        collectionSummary.push(
          `${totalSolCollected.toFixed(4)} SOL from ${walletsWithSol} wallets`,
        );
      }
      ...
      for (const [index, txBase64] of transactions.entries()) {
        try {
          setSuccessMessage(
            `Processing transaction ${index + 1} of ${totalTransactions}...`,
          );
          ...
          setProgressValue(progress);
          setSuccessMessage(
            `Processed: ${completedTransactions}/${totalTransactions} (${progress}%)`,
          );
        } catch (txError) {
          console.error("Transaction error:", txError);
  ...
  const collectSol = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/collect-sol`,
        {
          masterAddress: publicKey.toString(),
          network,
        },
      ...
      const { transactions, totalSolCollected, walletsWithSol } = response.data;
      ...
      setSuccessMessage(
        `Ready to collect ${totalSolCollected.toFixed(4)} SOL from ${walletsWithSol} wallets.`,
      );
  ...
  const updateBalancesInDatabase = async () => {
    try {
      ...
      const result = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/update-balances`,
        {
          addresses,
          solBalances,
          tokenBalances,
  ...
  const verifyBalances = async () => {
    ...
    try {
      ...
      setProgressValue(40);
      setSuccessMessage(
        `Checking ${tokenMints.size} token types in ${addresses.length} wallets...`,
      );
      ...
      for (const tokenMint of tokenMints) {
        try {
          await axios.post(
            `${process.env.REACT_APP_API_URL}/api/verify-token-balances`,
            {
          ...
          processedTokens++;
          const tokenProgress =
            40 + Math.floor((processedTokens / tokenMints.size) * 50);
          setProgressValue(tokenProgress);
          setSuccessMessage(
            `Verified ${processedTokens} of ${tokenMints.size} token types...`,
          );
        } catch (tokenError) {
          console.error(
            `Failed to verify token balances for ${tokenMint}:`,
            tokenError,
          );
        }
      }
  ...
  return (
    <Box sx={{ minHeight: "80vh", position: "relative" }}>
      <Typography
        variant="h2"
        sx={{ mb: 4, textAlign: "center", color: "#92E643" }}
      >
        Wallet Manager ({network})
      </Typography>
      ...
      <ChaosPaper>
                ...
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell
                      ...
                      sx={{
                        color:
                          getActiveTokenCount(wallets) > 0 ? "#92E643" : "#fff",
                  ...
                  {wallets
                    ...
                    .map((wallet) => {
                      ...
                      const totalTokenAmount =
                        calculateTotalTokenAmount(wallet);
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell
                            ...
                            sx={{
                              color:
                                primaryToken && primaryToken.amount > 0
                                  ? "#92E643"
                                  : "#fff",
                              borderBottom:
                                "1px solid rgba(255, 255, 255, 0.1)",
        ...
        >
              ...
              <>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Typography
                    ...
                    sx={{
                      fontWeight: "bold",
                      color: selectedWallet.solBalance > 0 ? "#92E643" : "#fff",
                    }}
                  >
                    {selectedWallet.solBalance
                      ? selectedWallet.solBalance.toFixed(6)
                      : "0"}{" "}
                    SOL
                  </Typography>
                </Box>
                        ...
                        {selectedWallet.tokens.map((token, index) => (
                          <TableRow key={index}>
                            ...
                            <TableCell
                              align="right"
                              sx={{
                                color: token.amount > 0 ? "#92E643" : "#fff",
                              }}
                            >
                            ...
                            <TableCell align="right" sx={{ color: "#92E643" }}>
                              {token.symbol}
                            </TableCell>
          ...
          <DialogActions
            sx={{ p: 2, borderTop: "1px solid rgba(146, 230, 67, 0.3)" }}
          >
      ...
      <Typography
        ...
        sx={{
          ...
          top: 10,
          right: 10,
          color: network === "mainnet-beta" ? "#FF6B6B" : "#92E643",
          fontWeight: "bold",
        }}
      >
